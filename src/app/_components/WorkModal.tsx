      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none overflow-hidden py-4 md:py-6 px-0 md:px-6 lg:px-8 xl:px-12"
...
function StoryModal({ work, onClose }: { work: Work; onClose: () => void }) {
  return (
    <div className="bg-gradient-to-br from-white via-grey-50 to-white rounded-none md:rounded-3xl shadow-2xl w-full h-full max-h-full overflow-hidden flex flex-col pointer-events-auto border-0 md:border border-grey-300">
      {/* Close Button */}
      <button
        onClick={onClose}
        className="absolute top-4 right-4 md:top-6 md:right-6 z-10 w-10 h-10 md:w-10 md:h-10 rounded-full bg-white/90 hover:bg-grey-100 flex items-center justify-center transition-colors border border-grey-300 shadow-lg focus:outline-none focus:ring-2 focus:ring-grey-500 focus:ring-offset-2"
        aria-label="Close modal"
      >
        <svg className="w-5 h-5 text-grey-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
        </svg>
      </button>

      {/* Scrollable Content */}
      <div className="overflow-y-auto flex-1 p-4 sm:p-6 md:p-8 lg:p-12 xl:p-16 pb-6 md:pb-8 lg:pb-12 xl:pb-16">
        {/* Hero Image Section */}
        <motion.div
        ...
        <div className="space-y-4 sm:space-y-6 md:space-y-8 mt-6 sm:mt-8 md:mt-12">
          ...
          {(work.summary || work.description) && (
            <motion.div
              ...
              className="bg-grey-50 rounded-lg md:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 border-l-4 border-grey-400 shadow-lg"
            >
          ...
          {work.artist && (
            ...
            >
              <p className="text-xs sm:text-sm text-grey-700 italic">{work.artist}</p>
            </motion.div>
          )}
...
function VideoRoomModal({ work, onClose }: { work: Work; onClose: () => void }) {
  ...
  return (
    <div className="bg-gradient-to-br from-white via-grey-50 to-white rounded-none md:rounded-3xl shadow-2xl w-full h-full max-h-full overflow-hidden flex flex-col pointer-events-auto border-0 md:border border-grey-300 relative">
      {/* Close Button */}
      <button
        onClick={onClose}
        className="absolute top-4 right-4 md:top-6 md:right-6 z-20 w-10 h-10 rounded-full bg-white/90 hover:bg-grey-100 flex items-center justify-center transition-colors border border-grey-300 shadow-lg focus:outline-none focus:ring-2 focus:ring-grey-500 focus:ring-offset-2"
        aria-label="Close modal"
      >
        <svg className="w-5 h-5 text-grey-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
      ...
      >
        ...
        <div className="flex h-full min-w-max">
          {/* Title Section */}
          <div className="w-screen h-full flex items-center justify-center px-4 sm:px-6 md:px-8 lg:px-16 py-4 md:py-0 scroll-snap-align-start relative">
            ...
            >
              <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-bold mb-3 sm:mb-4 bg-gradient-to-r from-grey-900 to-grey-600 bg-clip-text text-transparent">
                {work.title}
              </h1>
              {work.subTitle && (
                <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-grey-700 italic mb-4 sm:mb-6 md:mb-8">{work.subTitle}</p>
              )}
              <motion.div
                className="text-grey-700 text-sm sm:text-base md:text-lg"
                animate={{ x: [0, 10, 0] }}
          ...
          {/* Video Section */}
          <div className="w-screen h-full flex items-center justify-center px-4 sm:px-6 md:px-8 lg:px-16 py-4 md:py-0 scroll-snap-align-center bg-gradient-to-br from-grey-50/50 via-white/30 to-grey-50/50 relative">
            ...
            >
              {videoId ? (
                <div className="relative w-full pb-[56.25%] rounded-lg md:rounded-2xl overflow-hidden shadow-2xl border-2 md:border-4 border-grey-300 bg-black">
                  {/* Glow effect around video */}
                  <div className="absolute -inset-1 md:-inset-2 bg-grey-500/20 rounded-lg md:rounded-2xl blur-xl -z-10"></div>
                  <iframe
                ...
                <div className="relative w-full pb-[56.25%] rounded-lg md:rounded-2xl overflow-hidden shadow-2xl border-2 md:border-4 border-gray-700/50 bg-gray-800 flex items-center justify-center">
                  <p className="text-grey-700 text-sm sm:text-base">Video unavailable</p>
                </div>
              )}
            </motion.div>
          </div>
          ...
          {/* Description Section */}
          {work.summary && (
            <div className="w-screen h-full flex items-center justify-center px-4 sm:px-6 md:px-8 lg:px-16 py-4 md:py-0 scroll-snap-align-end">
              <motion.div
                initial={{ opacity: 0, x: 50 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.4 }}
                className="max-w-3xl bg-grey-50 rounded-lg md:rounded-2xl p-4 sm:p-6 md:p-8 lg:p-12 border border-grey-300 shadow-xl"
              >
      ...
      <div className="absolute bottom-4 sm:bottom-6 md:bottom-8 left-1/2 transform -translate-x-1/2 flex gap-2 z-10">
        <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-grey-500"></div>
        <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-grey-400"></div>
        {work.summary && <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-grey-400"></div>}
      </div>
    </div>
  );
}
